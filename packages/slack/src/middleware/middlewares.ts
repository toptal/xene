...
const middlewareContext = (payload): MiddlewareContext => {
  ...
  return { user, team, channel, token, callbackId, ephemeral: undefined,
...
const getResponse = async (handler: Handler, payload) => {
  ...
  if (ephemeralAdded) {
    Object.assign(response, isString(ephemeral) ? { text: ephemeral } : format.toSlack(ephemeral))
...
export const koa = async (handler: Handler, ctx: Koa.Context, next) => {
  ...
  // tslint:disable
  const payload = get(ctx, 'request.body.payload') || await streamPayload(ctx.req)
...
export const express = async (handler: Handler, req: Express.Request, res: Express.Response, next) => {
  ...
  const payload = get(req, 'body.payload') || await streamPayload(req)
  const body = await getResponse(handler, camelize(payload))
